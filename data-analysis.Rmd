...
```{r}
data <- data %>% mutate(area = str_split_fixed(rd.nm, ";", 2)[, 1], road = str_split_fixed(rd.nm, ";", 2)[, 2])
```

Getting road directions.

```{r}
data <- data %>% mutate(road_from = str_split_fixed(road, " To ", 2)[, 1], road_to = str_split_fixed(road, " To ", 2)[, 2])
```